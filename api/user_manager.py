...
class UserManager:
    ...
    async def create_or_get_user(self, telegram_id: int, tg_username: str = None):
        ...
        username=username,
        expire_at = datetime.etcnow() + timedelta(days=30),
        telegram_id=telegram_id
        )
            
        created_user: UserResponseDto = await self.client.users.create_user(body=user_data)
    ...
    async def renew_subscription(self, tg_id: int, days: int):
        ...
        logger.info(f"Updating subscription for user {user.id}, new expiration: {new_expires}")
